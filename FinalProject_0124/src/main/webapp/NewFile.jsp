        ...
        success : function(json) {        	
             for(var i in json){
	        	 ...
	        	 html += "<p class='step_add add_material'>새우6마리</p>";
	          	 ...
	          	 html += "<img src='http://localhost:8080/FinalProject/picture/"+ json[i]["rcseq"] +"/Step/"+json[i]["filename"]+"'>";	         	
	       		 html += "</div>";
	        	 html += "</div>";
        ...
        success : function(json) {        	
             for(var i in json){
	        	...
	        	html1 += "</div>";
	       		...
	       		html1 += "<img id='main_thumbs' src='http://localhost:8080/FinalProject/picture/" + json[i]["rcseq"] + "/Thumb/"+ json[i]["rcseq"] +"_thumb_1.jpg' alt='main thumb'>";
	     		html1 += "</div>";
			...
			for ( var i in data) {
				if (data[i]["rvfilename"]=="noImage,png"){
					...
					if (i < 3) {
						...
						if(data[i]["rvfilename"] === 'noImage'){
							result += "<img src='./images/noImage_100.png'>";
						}else{
							result += "<a href='javascript:viewLargePic(\"http://localhost:8080/FinalProject/rev_pic/"+ data[i]["rvseq"] + "/"	+ data[i]["rvfilename"]	+ "\")' class='reply_list_img'>";
							result += "<img src='./picture/rev_pic/"+data[i]["rvseq"]+"/"+ data[i]["rvfilename"] +"' alt='No Image'></a>";	
						}
						...
						result1 += "<a href='javascript:viewLargePic("http://localhost:8080/FinalProject/rev_pic/"	+ data[i]["rvseq"]	+ "/" + data[i]["rvfilename"] + "\")' class='reply_list_img'>";
		...
		$.ajax({
			...
			success : function(json) {
				...
				for(var i = 0; i < json.length ; i++){
					...
					result += "<li class='seen_thum'><div class='seen_thumb_img'><a href='recipe_view.do?rcseq="+rcseq+"'><img src='http://localhost:8080/FinalProject/picture/"+rcseq+"/Thumb/"+filename+"'/></a></div></li>";
				}
