...
<%
	...
	rankingTO to =(rankingTO) request.getAttribute("to");
	...
	/* 	 to.setRcseq(rs.getString("rcseq"));
        ...
        to.setAverstar(rs.getInt("allstar")); */

	//String rcseq = to.getRcseq();
	//String rfilename = piclist.get
	
	if(session.getAttribute("id")!=null && !session.getAttribute("id").equals(request.getRemoteAddr())){
		 System.out.println("sessiontest_ranking : " + login_id);
...
/* 	}else if(login_id.equals("admin")){
	...
	//liststart.append(" <li id='rankingList" +	(8*(rankPage)) + "' class='common_sp_list_li'>");
	liststart.append(" <li id='rankingList' class='common_sp_list_li'>");
	liststart.append("<p class='ranking_num st1'>");
	liststart.append("<b>"+rcseq+"</b>");
	liststart.append("	</p>");
	liststart.append("<div class='common_sp_thumb'>");
	liststart.append("<a href='./recipe_view.do?rcseq=" +rcseq + "' class='common_sp_link'>");
	
	liststart.append("if( url != ''){");
	liststart.append("<span class='common_vod_label'><img src='https://recipe1.ezmember.co.kr/img/icon_vod.png'></span>");

	liststart.append("<div class='rankThumb_Img'>");
	liststart.append("	<img src='./picture/" + rcseq + "/Thumb/"+rcseq +"_thumb_1.jpg'>");
	liststart.append("<!-- 	 <img src='./picture/" +rcseq + "/Thumb/"+rcseq +"_thumb_1.jpg'> -->");
	liststart.append("	</div>");
	liststart.append("	</a>");
	liststart.append("  </div>");
	liststart.append("  <div class='common_sp_caption'>");
	liststart.append("   <div class='common_sp_caption_tit line2'>");
	liststart.append("   <a href='./recipe_view.do?rcseq=" +rcseq + "'>");
	liststart.append(    subject );
	liststart.append("   </div>");
	liststart.append("   <div class='common_sp_caption_rv_name' style='display: inline-block; vertical-align: bottom;'>");
	liststart.append("<a href='/profile/recipe.html?uid=faces7'>");
	liststart.append("<img src='./picture" + rcseq + "'>");
//	liststart.append("<!-- 	/* 	result += "<img src='" + json.array[i-1]["picture"] + "'>"; */ -->");
	
	liststart.append("</a>	");			
	liststart.append("	</div>	");	
	

	liststart.append("	<div class='common_sp_caption_buyer' style='vertical-align: middle;'> 조회수 " +hit + " &nbsp;&nbsp; ");

	//liststart.append("<div class='common_sp_caption_buyer' style='vertical-align: middle;'> 평점 " +"0"+" &nbsp;&nbsp; ");
//	liststart.append("<div class='common_sp_caption_buyer' style='vertical-align: middle;'> 평점 " + allstar + " &nbsp;&nbsp; ");
            	...
            	<div id="contents_area_full">
            		...
            		<div class="rcp_m_list2">
	            		...
	            		<ul id="rankingListUl" class="common_sp_list_ul ea4" style="padding: 40px 0 0 8px;">
	            			...
	            			<%=liststart %>
	      <!--       			
	            <li id='rankingList" + i+(8*(rankPage)) + "' class='common_sp_list_li'>
				<p class='ranking_num st1'>
				<b>"+rankNum+"</b>
				</p>";
				<div class='common_sp_thumb'>";
				<a href='./recipe_view.do?rcseq=" + json.array[i-1]["rcseq"] + "' class='common_sp_link'>
				
				if( url != ""){
				<span class='common_vod_label'><img src='https://recipe1.ezmember.co.kr/img/icon_vod.png'></span>
				
			<div class='rankThumb_Img'>
				<img src='./picture/" + json.array[i-1]["rcseq"] + "/Thumb/"+ json.array[i-1]["rcseq"] +"_thumb_1.jpg'>
			 <img src='D:/workspace/FinalProject_1031_0900/src/main/webapp/picture/" + json.array[i-1]["rcseq"] + "/Thumb/"+ json.array[i-1]["rcseq"] +"_thumb_1.jpg'>
				</div>
				</a>
                </div>";
                <div class='common_sp_caption'>
                <div class='common_sp_caption_tit line2'>
                <a href='./recipe_view.do?rcseq=" + json.array[i-1]["rcseq"] + "'>
                result += json.array[i-1]["subject"];
                </div>";
                <div class='common_sp_caption_rv_name' style='display: inline-block; vertical-align: bottom;'>
			<a href='/profile/recipe.html?uid=faces7'>
				<img src='" +  + "'>
			/* 	result += "<img src='" + json.array[i-1]["picture"] + "'>"; */
				
			</a>";				
				</div>		
				
				if(listKind==='hit'){
					<div class='common_sp_caption_buyer' style='vertical-align: middle;'> 조회수 " + json.array[i-1]["hit"] + " &nbsp;&nbsp; 
				}else{
				<div class='common_sp_caption_buyer' style='vertical-align: middle;'> 평점 " + json.array[i-1]["allstar"] + " &nbsp;&nbsp; 
				}
	...
	<div id="sidebar">
		<div class="inner">
			...
			<section>
				<header class="major">
					<h2>내가 본 레시피</h2>
					<ul class="seen_thumbnail" id="seen_List">
				<%-- 
					<li class='seen_thum'><div class='seen_thumb_img'>
					<a href='recipe_view.do?rcseq="+<%=rcseq %>+"'>
					
				<!-- 	<img src="./images/noImage_300.png"/></a></div></li> -->
					
					<img src="./picture/'+<%=rcseq %>+'/Thumb/'+<%=rfilename %>>+'"/></a></div></li>
					 --%>
		...
		success : function(json) {
			...
			for(var i = 0; i < json.length ; i++){
				...
				result += "<li class='seen_thum'><div class='seen_thumb_img'><a href='recipe_view.do?rcseq="+rcseq+"'><img src='./picture/"+rcseq+"/Thumb/"+filename+"'/></a></div></li>"; 
																																								/* <img src='http://localhost:8080/FinalProject/picture/"+rcseq+"/Thumb/"+filename+"'/></a></div></li>"; */
	/	}
